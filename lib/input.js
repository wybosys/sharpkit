...
/**
 * Fast access to (uncached) image bbx.
 * A Promises/A+ promise is returned when `callback` is not provided.
 *
 * @example
 * const image = sharp(inputJpg);
 ...
 *   });
 *
 * @param {Function} [callback] - called with the arguments `(err, bbx)`
 * @returns {Promise<Object>|Sharp}
 */
function bbx (tolerance, callback) {
  const that = this;
  that.options.tolerance = tolerance;
  if (is.fn(callback)) {
    if (this._isStreamInput()) {
      this.on('finish', function () {
        that._flattenBufferIn();
        sharp.bbx(that.options, callback);
      });
    } else {
      sharp.bbx(this.options, callback);
    }
    return this;
  } else {
    if (this._isStreamInput()) {
      return new Promise(function (resolve, reject) {
        that.on('finish', function () {
          that._flattenBufferIn();
          sharp.bbx(that.options, function (err, bbx) {
            if (err) {
              reject(err);
            } else {
              resolve(bbx);
